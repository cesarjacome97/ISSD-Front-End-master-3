...
const IndeterminadasPage = props => {
    ...
    const handleOpenRow = (row) => {
		console.log(row)
		setSdata(row)
		console.log(isOpenSaveConfirmation)
		setIsOpenSaveConfirmation(true)
		//openSaveConfirmation(row)
        /*history.push({
		...
		AddressServices.buscaxml().then((res1) => {
			...
			for(var ii=0;ii<value.length;ii++)
			{
				...
				if(value[ii].innerHTML.includes('id_domicilio'))
				{
					...
					var dd = iddomicilio[0]
				}
			}
			console.log()
			AddressServices.setInd(sdata['ID_DOMICILIO_PR'],dd).then((res) => {
				console.log(res)
				handleShowSnackBar('success', 'Se actualizó correctamente')
			})
	...
	const onSaveClick = () => {
		...
		if(res.data[0])
		{
			if(res.data[0]['EDO_NORMALIZACION']==='E')
			{
				console.log(res.data)
				setSdata(res.data)
				console.log(isOpenSaveConfirmation)
			...
			handleShowSnackBar('warning', 'La dirección no es Indeterminada')
		}
		else
		...
		AddressServices.buscaxml().then((res1) => {
			...
			setTimeout(function () {
				...
				const query = {
					...
					console.log(iddomicilio[0])
            AddressServices.getInd(iddomicilio[0]).then((res) => {
            ...
            <div className="section section-basic" id="basic-elements">
                ...
                <Container style={{ marginBottom: -20, maxWidth: "90%" }}>
                    <Breadcrumb>
                        ...
                        <BreadcrumbItem active>Indeterminadas</BreadcrumbItem>
                    </Breadcrumb>
                    <h3 className="d-none d-sm-block text-center">
                        Direcciones Indeterminadas Actuales
                    </h3>
					    <Input
                        type="text"
                        name="search"
                        ...
                        currentTableData.length > 0 ?
                            ...
                            <PaperStyled>
                                <h6 className="d-none d-sm-block text-center" style={{ padding: 50 }}>
                                    No hay direcciones indeterminadas
                                </h6>
                            </PaperStyled>
                    }
                </Container>
				<Dialog
                    ...
                    PaperComponent={() =>
                        ConfirmationDialog({
                            title: 'Confirmación',
                            message: '¿Está seguro de marcar la direccion como excepcionada?',
                            confirmButton: true,
                            cancelButton: true,
...
export default IndeterminadasPage;